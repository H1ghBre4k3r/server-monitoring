...
async fn test_multiple_collectors_single_alert_actor() {
    ...
    let collector1 = CollectorHandle::spawn(config1, metric_tx.clone(), tokio::sync::broadcast::channel(16).0);
    let collector2 = CollectorHandle::spawn(config2, metric_tx.clone(), tokio::sync::broadcast::channel(16).0);
