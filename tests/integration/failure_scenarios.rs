...
async fn test_collector_handles_agent_unreachable() {
    ...
    let collector_handle = CollectorHandle::spawn(config, metric_tx.clone(), tokio::sync::broadcast::channel(16).0);
...
async fn test_system_continues_after_collector_error() {
    ...
    let working_collector = CollectorHandle::spawn(working_config, metric_tx.clone(), tokio::sync::broadcast::channel(16).0);
    let failing_collector = CollectorHandle::spawn(failing_config, metric_tx.clone(), tokio::sync::broadcast::channel(16).0);
